...
export type SelectUseState<S> = <A>(f: (s: S) => A) => [A, Dispatch<SetStateAction<A>>]

const useStateRef = <R, S>(stateRef: ZIO<R, never, ZIORef<S>>): ZIO<R, never, SelectUseState<S>> => pipe(
  stateRef,
...
const forComponent = <R, S>(stateRef: ZIO<R, never, ZIORef<S>>): <A>(f: (fc: [R, SelectUseState<S>]) => A) => ZIO<R, never, A> =>
  f => pipe(
